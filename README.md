...
本工程以 AMD Ryzen 5 5500U (6C/12T) + Ubuntu 24.04 为实验平台，从零构建了一套递进式四层分区隔离方案。通过组合 Linux 内核的 cgroups v2、SCHED_DEADLINE、CMA 以及硬件 CAT/QoS 等机制，在强干扰背景负载下将系统抖动从 **10085 μs 降低至 63 μs**（降幅约 99.4%）。
...
graph TB
    ...
    style L4 fill:#fce4ec,stroke:#d32f2f
```
...
| 需求 | 对应文档 |
| :--- | :--- |
| 了解完整操作流程与踩坑记录 | [分区.md](分区.md)（实验日志，约 1500 行） |
